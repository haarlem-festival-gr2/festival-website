...
<div class="p-8 relative bg-[#fdbef5] text-center">
    ...
    <button onclick="speakText()" class="absolute bottom-10 right-10 bg-transparent border-none text-4xl">
        🔊
    </button>
</div>

<div class="text-center py-4">
    <h2 class="text-4xl font-noto-serif">Festival in Haarlem 2023 schedule</h2>
</div>

@foreach ($jazzDaysWithPerformances as $dayWithPerformances)
    <div class="p-4 bg-[#B92090] text-center">
        <h3 class="text-2xl text-white font-bold">DAY {{ $dayWithPerformances['day']->getDayNumber() }} - {{ date('l, F jS', strtotime($dayWithPerformances['day']->getDate())) }} 📍
            <span class="tooltip"><u>{{ $dayWithPerformances['venue']->getVenueName() }}</u>
    ...
    <div class="relative">
        <img src="{{ $dayWithPerformances['day']->getImgPath() }}" alt="Jazz Day Image" class="w-full filter brightness-70">
        @foreach ($dayWithPerformances['performances'] as $index => $performance)
            <div style="position: absolute; top: {{ 40 + ($index * 100) }}px; left: 0; width: 100%; padding: 8px; box-sizing: border-box;">
                <div class="flex justify-between items-center mb-4" style="background: rgba(0,0,0,0.5); padding: 10px;">
                    <p class="text-white">{{ $performance->getArtist()->getArtistName() }}<br>{{ date('H:i', strtotime($performance->getStartDateTime())) }} - {{ date('H:i', strtotime($performance->getEndDateTime())) }}
                        @if(trim($performance->getHall()) !== '')
                            | {{ $performance->getHall() }}
                        @endif
                        @if($performance->getPrice() != '0.00')
